...
interface State {
  animeLists: AnimeData[] | undefined;
  selectedAnime: AnimeData | null;
}

type PropsFromRedux = ConnectedProps<typeof connector>;

const dummyAnimeData: AnimeData = {
  mal_id: 1,
...
const Button = (props: PropsFromRedux) => {
  const dispatch = useDispatch();

  const onClick = () => {
    dispatch(setAnimeListAction([dummyAnimeData]));
  };

  return (
    <div>
      {props.animeLists?.map(item => (
      ...
      <button
        type="button"
        className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800"
        onClick={() => dispatch(setAnimeListAction([]))}
      >
        Remove Anime Lists
      </button>
    </div>
